...
\begin{example}
  ...
  \begin{problem}
    ...
    \end{selectAll}
    \begin{hint}
      $f$ is increasing where $f'(x)>0$, or above the $x$-axis.%, i.e. on the intervals
      %$(-\infty, 0)$ and $(2,3)$.
    \end{hint}
  \end{problem}
  ...
  \begin{problem}
    ...
    \begin{hint}
      $f$ has a critical point at the zeros of $f'$ (where $f$ crosses the $x$-axis, and the places where $f'$ does not exist.  %In this case, $x=0$, $x=2$, and $x=3$.
    \end{hint}
  \end{problem}
...
\end{selectAll}
    ...
    \begin{hint}
      A point of inflection occurs when the concavity of $f$ changes. Since we don't have a graph of $f$, we need to use $f'$.  Remember that if $f'$ is increasing, then the derivative of $f'$ ($f''$) must be positive, so then $f$ must be concave up. % This is reflected in the sign of $f''$ changing.  This only occurs at one point in this graph, namely $x=1$.
    \end{hint}
  ...
  \begin{problem}
    ...
    \begin{hint}
      $f$ is concave down when $f''(x)<0$.  Use the slope of $f'$ to inform you about $f''$.%This occurs for $x<1$ on this graph.  So the correct answer is to select both $(-\infty, 0)$ and $(0,1)$. 
    \end{hint}
  \end{problem}
